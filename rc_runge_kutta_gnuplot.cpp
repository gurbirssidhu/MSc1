...
double slope(double time, double value) {
...
int main() {    
    // parameters
    double q0{5.0};         // initial current on capacitor
    double h{0.3};          // step difference
    double init_time{0};
    ...
    for ( int i{0}; i < size - 1; ++i) {
        // using euler method
        charge_eul[i+1] = charge_eul[i] + h * slope(time[i], charge_eul[i]);  

        // using rk method-- RK4
        double k1{ slope(time[i], charge_rg[i]) };
        double k2{ slope(time[i] + h/2.0, charge_rg[i] + h * k1 / 2.0) };
        double k3{ slope(time[i] + h/2.0, charge_rg[i] + h * k2 / 2.0) };
        double k4{ slope(time[i] + h, charge_rg[i] + h * k3) };
